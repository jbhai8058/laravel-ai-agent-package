...
class AiAgentManager
{
        ...
        try {
            if (config('ai-agents.database.auto_load_schema', true)) {
                ...
                $this->databaseService = new AiDatabaseService(
                    $this->databaseSchema, $this->getCurrentDriver()
                );
            }
        } catch (\Exception $e) {
    ...
    protected function addSchemaContextToMessages(array $messages, string $schemaContext): array
    {
        ...
        array_unshift($messages, [
            'role'    => 'system',
            'content' => "You are an AI assistant with access to the following database schema. " . "Use this information to provide accurate and relevant responses.\n\n" . $schemaContext
        ]);
